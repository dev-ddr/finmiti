"""Module consisting of the DuckDB/parquet-backed stock data handler."""
...
class StockDDB(Stock):
    """DuckDB/parquet-backed Stock: data lives in per-stock parquet files on local disk,
    with a JSON manifest sidecar tracking coverage and freshness.
    """
    ...
    @staticmethod
    def from_folder(folder) -> Optional["StockDDB"]:
        """Reconstructs a StockDDB from a stored data folder (via its manifest, else the folder name)."""
        folder = Path(folder)
        ...
        try:
            return StockDDB(symbol, EXCHANGE(exch), EXCHANGE_TYPE(exch_type))
        except Exception:
            return None
